...
describe("LlmProvider interface", () => {
  ...
  it("DeepSeekProvider has correct name", () => {
    const p = new DeepSeekProvider("test");
...
describe("DeepSeekProvider", () => {
  ...
  it("call returns text", async () => {
    ...
    const p = new DeepSeekProvider("ds_test", "deepseek-test");
    const result = await p.call("prompt", 256);
    expect(result).toBe("Hello from DeepSeek");
  });

  it(
    "uses DEEPSEEK_MODEL env",
    withEnv({ DEEPSEEK_MODEL: "deepseek-v4" }, () => {
  ...
  it("throws on empty response", async () => {
    ...
    const p = new DeepSeekProvider("k");
    await expect(p.call("prompt", 100)).rejects.toThrow("Unexpected empty response from deepseek");
  });
});
...
describe("createProvider", () => {
  ...
  it(
    "creates deepseek provider",
    withEnv({ DEEPSEEK_API_KEY: "ds_test" }, () => {
