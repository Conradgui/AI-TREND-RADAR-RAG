...
1. 预热后的镜像先启动 Web 服务；
2. 首次运行先摄取仓库已附带的公开日报，最新日期优先；
3. 再从上游公开站点单向检查报告变化，并摄取新增或变更日期；
4. 每成功处理一天就写入本地检查点；中断后只补未完成日期；
5. 同步失败时继续使用最后一次成功的本地索引。

app 运行期间还会按 `RAG_CORPUS_UPDATE_INTERVAL_SECONDS`（默认 6 小时）重复检查；
这项轮询与启动检查共用同一个进程内更新入口和写锁，不需要额外启动更新容器。
GitHub Actions 只负责把经过校验的公开语料发布到仓库，不会连接用户本机的 Neo4j 或 ChromaDB。
若电脑关机，轮询自然暂停；下次启动 app 时会立即补查，不需要手动重建容器或数据卷。

`RAG_CORPUS_RECHECK_DAYS=30` 是近期修订重检窗口，不是“只保留 30 天语料”的限制。若本地长期未启动，更新程序会追平缺失日期。

### 首次索引期间可以做什么

页面和 System 面板会先可访问；System 中“语料同步：正在建立本地索引”表示后台正在建立或补齐索引。此时可以浏览日报，Agent 只会基于已建立的证据回答。最新日报优先进入索引，历史语料随后补齐；不要把“同步中”误解成服务已经卡死。若上游暂时不可达但本地日报已可检索，面板会显示“上游同步失败（本地语料可用）”。
