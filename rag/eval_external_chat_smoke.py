"""Run one live chat smoke with external Tavily evidence enabled."""
...
DEFAULT_OUTPUT = Path("docs/rag-transformation/evals/external-chat-smoke-2026-06-22.json")
DEFAULT_QUESTION = (
...
async def build_external_chat_smoke(question: str) -> dict:
    """Run one vector-only chat response with external search enabled."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one external-evidence chat smoke.")
    parser.add_argument("--question", default=DEFAULT_QUESTION)
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    args = parser.parse_args()

    smoke = asyncio.run(build_external_chat_smoke(args.question))
