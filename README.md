...
完成后打开 [http://127.0.0.1:8001](http://127.0.0.1:8001)。首次语料索引会在后台继续：**仓库已附带的最新日报优先可检索，历史报告随后补齐**；之后才检查上游是否有更新。即使上游暂时不可达，System 面板也会显示“上游同步失败（本地语料可用）”，Agent 仍只基于已建立的证据回答。
